...
export const assignTasksFunctionDescription =
      ...
      items: {
        ...
        properties: {
          ...
          description: {
            type: 'string',
            description:
              'Concise description of the essential task to be performed',
          },
          tool: {
            type: 'string',
            description: 'Specific tool to be used for the task, if applicable',
          },
        },
        required: ['member', 'description'],
      },
      description:
        'List of essential tasks to be assigned, maximum 5 tasks per turn.',
    },
  },
  required: ['tasks'],
};

export const endProcessFunctionDescription =
...
export const endProcessFunctionParameters = {
  ...
  properties: {
    reason: {
      type: 'string',
      description: 'Brief explanation of why the process is ending',
    },
  },
  required: ['reason'],
};
...
/**
 * Get the task manager prompt from config with fallback to default.
 */
export async function getTaskManagerPrompt(): Promise<string> {
  return getPrompt(['agents', 'taskManager', 'prompt'], taskManagerPrompt);
}

/**
 * Get the assign tasks function description from config with fallback to default.
 */
export async function getAssignTasksDescription(): Promise<string> {
  return getPrompt(
    ['agents', 'taskManager', 'assignTasksDescription'],
    assignTasksFunctionDescription
  );
}

/**
 * Get the end process function description from config with fallback to default.
 */
export async function getEndProcessDescription(): Promise<string> {
  return getPrompt(
    ['agents', 'taskManager', 'endProcessDescription'],
